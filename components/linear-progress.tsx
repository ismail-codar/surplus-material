...
export const MdcLinearProgress = (props: MdcLinearProgressProps) => {
  const dom = (
          ...
          classes: [
            {
              ...
              'mdc-linear-progress--indeterminate': sDataValue(
                props.indeterminate
              ),
    ...
    linearProgress = window['mdc'].linearProgress.MDCLinearProgress.attachTo(
      dom
    );
  ...
  if (props.progress)
    ...
    else linearProgress.progress = props.progress;
  if (props.buffer)
    if (typeof props.buffer == 'function')
      S.on(props.buffer, () => {
        linearProgress.buffer = (props.buffer as any)();
      });
    else linearProgress.buffer = props.buffer;
  return dom;
};
